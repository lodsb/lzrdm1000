...
//			reportError( e1 );
	...
	private void createServerWindow()
	{
		...
		spf.setDefaultCloseOperation( JFrame.DO_NOTHING_ON_CLOSE );
	}
	...
	private void createControlWindow()
	{
		...
		ez	= new EZSlider[ ctrlNames.length ];
		...
		for( int i = 0; i < ctrlNames.length; i++ ) {
			ez[ i ] = new EZSlider();
			ez[ i ].setControlSpec( ctrlSpecs[ i ]);
			ez[ i ].setLabel( ctrlNames[ i ]);
			ez[ i ].setValue( ctrlSpecs[ i ].getDefaultVal() );
			lay.setConstraints( ez[ i ], con );
			p.add( ez[ i ]);
			new ControlListener( ez[ i ]);
		}
	...
	private void createSynth()
	{
		...
		try {
			defName			= "MotoRevCtrl";
			ctrlValues		= new float[ ez.length ];
			for( int i = 0; i < ctrlValues.length; i++ ) {
				ctrlValues[ i ] = (float) ez[ i ].getValue();
			}
...
// ------------- internal classes -------------
	...
	implements ActionListener
	{
		...
		private ControlListener( EZSlider ez )
		{
			this.ez = ez;
